...
Commit both files to git. When you change your contract, run `bun prisma-cli contract emit` to update them.
...
```bash
bun prisma-cli contract emit       # Update contract.json and contract.d.ts
bun prisma-cli db init             # Create tables in the database
bun prisma-cli migration status    # Show migration status
```
...
2. Run `bun prisma-cli contract emit` to regenerate the contract.
...
- **Catalogs.** When the workspace's `pnpm-workspace.yaml` defines a `catalogs` entry for `@prisma/cli` or `@prisma/orm-postgres`, pnpm uses the catalog version everywhere — `init` does too. If you wanted the published `latest` instead, update or remove the catalog entry, then re-run `pnpm install`.
